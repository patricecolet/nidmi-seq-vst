...
NidmiSeqAudioProcessorEditor::NidmiSeqAudioProcessorEditor(NidmiSeqAudioProcessor& p)
    ...
    // GRAMMAIRE À SIX MOLETTES (CONCEPTION §2). Elle REMPLACE les cinq encodeurs
    // contextuels dont le rôle changeait selon la page : ceux-là sont supprimés, pas
    // masqués. Du code mort qu'on garde « au cas où » finit par diverger de ce qui
    // s'affiche — c'est la panne qu'on a passé la journée à traquer.
    ...
    for (int i = 0; i < encoders::kCount; ++i) {
        gLab_[i].toFront(false);
        gEnc_[i].toFront(false);
        gPush_[i].toFront(false);
    }
...
NidmiSeqAudioProcessorEditor::~NidmiSeqAudioProcessorEditor() {
    ...
    for (int i = 0; i < encoders::kCount; ++i)
        gPush_[i].setLookAndFeel(nullptr);
}
...
void NidmiSeqAudioProcessorEditor::timerCallback() {
    ...
    if (encSyncCooldown_ > 0) --encSyncCooldown_;
    if (masterHudFrames_ > 0)  --masterHudFrames_;
    // Ce qui suivait ici resynchronisait à chaque frame les étiquettes des cinq
    // encodeurs contextuels, page par page et focus par focus. La grammaire à six
    // molettes rend tout ce bloc caduc : `refreshGrammarEncoders()` relit une seule
    // source — `encoders::describe()` — et il n'y a plus d'étiquette à deviner.
