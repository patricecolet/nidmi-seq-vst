...
9. **Depuis un contexte ouvert**, seuls deux appuis répondent : celui de la molette
   poussée, qui ressort, et celui de `Row`, qui bascule vers son propre contexte —
   celui de la row, ou celui du **sous-pattern** quand on est entré dedans, puisque
   c'est lui l'objet courant à ce niveau.
10. **Le clavier suit le niveau.** En saisie pas à pas, les blanches sont les pas de la
    row — et les **sous-pas** dès qu'on est entré dans un pas. Le même geste édite
    toujours l'objet où l'on se trouve.
11. Une **marque permanente** sur les molettes qui ont une profondeur : on ne pousse
    jamais pour voir.
12. **Les contextes peuvent différer selon le mode d'affichage de la row** — travailler
